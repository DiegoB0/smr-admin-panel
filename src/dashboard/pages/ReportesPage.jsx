...
import { Package, DollarSign, FileText, Tag, AlignLeft, Ruler, Image, Search, CircleX, CircleCheck } from 'lucide-react'

function ReportesPage() {
  const { createProducto, updateProducto } = useProductos()
  const { listReportes } = useRequisiciones()
                  ...
                  {productos.length > 0 ? (
                    productos.map((p) => (
                      <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{p.creadoPor?.email || "no se ha revisado"}</td>
